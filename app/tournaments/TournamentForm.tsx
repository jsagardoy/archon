...
const TournamentForm = ({ handleClose }: Props) => {
  ...
  const insertTournamentInDatabase = async (newTournament: Tournament) => {
    try {
      const resp = await addTournament(newTournament)
      if (!resp) {
    ...
    >
      <FormControl sx={{ gap: '1rem', m: '1rem' }} error={error}>
        ...
        <FormControl fullWidth>
          ...
          <Select
            error={validationResult?.tournamentType?._errors !== undefined}
        ...
        />
        <TextField
          error={validationResult?.numberOfRounds?._errors !== undefined}
          helperText={
            validationResult?.numberOfRounds?._errors.join(', ') || ''
          }
          required
        ...
        />
        <TextField
          error={validationResult?.maxNumberOfPlayers?._errors !== undefined}
          helperText={
            validationResult?.maxNumberOfPlayers?._errors.join(', ') || ''
          }
          required
